...
def init(context):
    ...
    for index, row in data.iterrows():
        ...
        super_v_list.append((close-y_close) * volume);
        if temp_index > 0 :
            s1 = (data["close"][index-1] - data["close"][index-2]);
            s2 = (data["close"][index-2] - data["close"][index-3]);
            super_v_list.append((s1-s2) * volume);

            # if s1 >= s2 >= 0:
            #     super_v_list.append((s1-s2) * volume);
            # elif s1 < 0 or s2 < 0 :
            #     super_v_list.append(-1);
            # else:
            #     super_v_list.append(-1);
        else :
            super_v_list.append(0)
    ...
    context.obv_rate_list = obv_rate_list;
    context.qhlsr_list = qhlsr_list;
    context.super_v_list = super_v_list;
    print(qhlsr_list)
...
def handle_bar(context, bar_dict):
    ...
    qhlsr_list = context.qhlsr_list;
    up_down_list = context.up_down_list;
    obv_rate_list = context.obv_rate_list;
    super_v_list = context.super_v_list;
    up_list = context.up_list;

    # super_v_list = ta.MA(np.array(super_v_list[0:len(super_v_list)]), timeperiod=3)
    # up_down_list = ta.MA(np.array(up_down_list[0:len(up_down_list)]), timeperiod=3)

    super_v_list = ta.EMA(np.array(super_v_list), timeperiod=5)
    up_down_list = ta.EMA(np.array(up_down_list), timeperiod=5)

    index = (data_size - 1);

    if index > 2 :
        flag1 = (super_v_list[index] > 0);
        flag11 = ((up_down_list[index] - up_down_list[index - 1]) > (up_down_list[index - 1] - up_down_list[index - 2]) > 0);
        # flag111 = (up_down_list[index] > up_down_list[index-1] > up_down_list[index-2] > 0);
        flag1111 = (up_down_list[index] > 3);

        flag2 = (super_v_list[index] < 0);
        flag22 = (up_down_list[index] < 5);
        ...
        flag7 = (obv_rate_list[index] < 1.1);
        flag8 = (qhlsr_list[index] < 0);
